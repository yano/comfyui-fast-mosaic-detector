...
def _rect_sum_grid_torch(
    ...
    """Vectorized integral lookup supporting batched tensors."""
    ...
    if integral.ndim != 3:
        raise ValueError("Expected integral tensor with 2 or 3 dimensions")

    batch_indices = (
        torch.arange(integral.shape[0], device=integral.device)
        .view(-1, 1, 1)
        .expand_as(y0)
    )

    sum_val = (
        integral[batch_indices, y1, x1]
        - integral[batch_indices, y0, x1]
        - integral[batch_indices, y1, x0]
        + integral[batch_indices, y0, x0]
    )
    area = (x1 - x0) * (y1 - y0)
    return sum_val, area
...
def _estimate_mosaic_period_torch(
    binary_map: torch.Tensor, config: MosaicDetectionConfig
) -> torch.Tensor:
    """Estimate the mosaic period using simple overlap histograms on GPU."""

    if binary_map.ndim == 2:
        binary_map = binary_map.unsqueeze(0)
        squeeze_result = True
    elif binary_map.ndim == 3:
        squeeze_result = False
    else:
        raise ValueError("Expected tensor with shape (H, W) or (N, H, W)")

    with torch.inference_mode():
        binary_bool = binary_map > 0
        batch, height, width = binary_bool.shape

        candidates = torch.arange(
            config.mosaic_length_min,
            config.mosaic_length_max + 1,
            device=binary_map.device,
            dtype=torch.int32,
        )

        if candidates.numel() == 0:
            periods = torch.zeros(batch, device=binary_map.device, dtype=torch.int32)
        else:
            counts_x = torch.zeros(
                batch, candidates.numel(), device=binary_bool.device, dtype=torch.float32
            )
            counts_y = torch.zeros_like(counts_x)

            for idx, k in enumerate(candidates):
                k_int = int(k.item())
                if k_int <= 0 or k_int >= width:
                    continue
                overlap_x = (
                    binary_bool[:, :, k_int:] & binary_bool[:, :, :-k_int]
                ).sum(dim=(1, 2))
                counts_x[:, idx] = overlap_x.to(dtype=torch.float32)

            for idx, k in enumerate(candidates):
                k_int = int(k.item())
                if k_int <= 0 or k_int >= height:
                    continue
                overlap_y = (
                    binary_bool[:, k_int:, :] & binary_bool[:, :-k_int, :]
                ).sum(dim=(1, 2))
                counts_y[:, idx] = overlap_y.to(dtype=torch.float32)

            scores = counts_x * counts_y
            best_scores, best_idx = scores.max(dim=1)
            best_periods = candidates[best_idx]

            best_counts_x = counts_x.gather(1, best_idx.unsqueeze(1)).squeeze(1)
            best_counts_y = counts_y.gather(1, best_idx.unsqueeze(1)).squeeze(1)

            pixel_mass = binary_bool.sum(dim=(1, 2)).to(dtype=torch.float32)
            min_mass = (
                torch.tensor(float(config.mosaic_length_min**2), device=binary_map.device)
            )
            validity = (
                (best_scores > 0.0)
                & (best_counts_x > 0.0)
            ...
            periods = torch.where(validity, best_periods, torch.zeros_like(best_periods))

    if squeeze_result:
        periods = periods.squeeze(0)

    return periods.to(dtype=torch.int32)


def _assemble_mosaic_mask_torch(
    feature_map: torch.Tensor, tile_sizes: torch.Tensor, config: MosaicDetectionConfig
) -> torch.Tensor:
    """Dilate detected features around the estimated tile size on GPU."""

    if feature_map.ndim == 2:
        feature_map = feature_map.unsqueeze(0)
        tile_sizes = tile_sizes.unsqueeze(0)
        squeeze_result = True
    elif feature_map.ndim == 3:
        squeeze_result = False
    else:
        raise ValueError("Expected feature map tensor with shape (H, W) or (N, H, W)")
    ...
    with torch.inference_mode():
        ...
        flat_sizes = tile_sizes.reshape(-1)

        for idx in range(batch):
            tile = int(flat_sizes[min(idx, flat_sizes.numel() - 1)].item())
            if tile <= 0:
                masks.append(
                    torch.zeros((height, width), device=feature_map.device, dtype=torch.float32)
                )
                continue

            kernel = max(1, min(tile, height, width))
            fmap = feature_map[idx : idx + 1].unsqueeze(1)
            dilated = F.max_pool2d(
                fmap, kernel_size=kernel, stride=1, padding=kernel // 2
            )
            dilated = dilated[..., :height, :width]

            smooth_kernel = max(1, kernel // 2)
            if smooth_kernel > 1:
                smoothed = F.avg_pool2d(
                    dilated, kernel_size=smooth_kernel, stride=1, padding=smooth_kernel // 2
                )
                smoothed = smoothed[..., :height, :width]
            else:
                smoothed = dilated

            mask = (smoothed.squeeze(0).squeeze(0) > 0.05).to(torch.float32)
            masks.append(mask)
...
def _get_mosaic_feature_map_torch(
    ...
    if v_channel.dtype != torch.float32:
        v_channel = v_channel.to(dtype=torch.float32)

    if v_channel.ndim == 2:
        v_channel = v_channel.unsqueeze(0)
        squeeze_result = True
    elif v_channel.ndim == 3:
        squeeze_result = False
    else:
        raise ValueError("Expected tensor with shape (H, W) or (N, H, W)")

    batch, height, width = v_channel.shape

    grad_h = torch.zeros_like(v_channel)
    grad_v = torch.zeros_like(v_channel)

    grad_h[:, :-1, :] = torch.abs(v_channel[:, :-1, :] - v_channel[:, 1:, :])
    grad_v[:, :, :-1] = torch.abs(v_channel[:, :, :-1] - v_channel[:, :, 1:])

    integral_h = torch.cumsum(torch.cumsum(grad_h, dim=1), dim=2)
    integral_v = torch.cumsum(torch.cumsum(grad_v, dim=1), dim=2)

    integral_h = F.pad(integral_h.unsqueeze(1), (1, 0, 1, 0)).squeeze(1)
    integral_v = F.pad(integral_v.unsqueeze(1), (1, 0, 1, 0)).squeeze(1)
    ...
    x_range = torch.arange(2 + pw1, width - 3 - pw1, device=v_channel.device)
    y_range = torch.arange(2 + pw1, height - 3 - pw1, device=v_channel.device)

    if x_range.numel() == 0 or y_range.numel() == 0:
        return output.squeeze(0) if squeeze_result else output

    X = x_range.view(1, 1, -1).expand(batch, -1, -1)
    Y = y_range.view(1, -1, 1).expand(batch, -1, -1)

    def _clipped_bounds(offset_x0: int, offset_x1: int, offset_y0: int, offset_y1: int):
        x0 = torch.clamp(X + offset_x0, 0, width).to(torch.long)
        x1 = torch.clamp(X + offset_x1, 0, width).to(torch.long)
        y0 = torch.clamp(Y + offset_y0, 0, height).to(torch.long)
        y1 = torch.clamp(Y + offset_y1, 0, height).to(torch.long)
        return x0, x1, y0, y1

    x0, x1, y0, y1 = _clipped_bounds(-pw2, pw2 + 1, -(2 + pw1), -2)
    sum1h, area1h = _rect_sum_grid_torch(integral_h, y0, y1, x0, x1)

    x0, x1, y0, y1 = _clipped_bounds(-pw2, pw2 + 1, 3, 3 + pw1)
    sum2h, area2h = _rect_sum_grid_torch(integral_h, y0, y1, x0, x1)

    x0, x1, y0, y1 = _clipped_bounds(-pw2, pw2 + 1, 0, 1)
    sum3h, area3h = _rect_sum_grid_torch(integral_h, y0, y1, x0, x1)

    x0, x1, y0, y1 = _clipped_bounds(-(2 + pw1), -2, -pw2, pw2 + 1)
    sum1v, area1v = _rect_sum_grid_torch(integral_v, y0, y1, x0, x1)

    x0, x1, y0, y1 = _clipped_bounds(3, 3 + pw1, -pw2, pw2 + 1)
    sum2v, area2v = _rect_sum_grid_torch(integral_v, y0, y1, x0, x1)

    x0, x1, y0, y1 = _clipped_bounds(0, 1, -pw2, pw2 + 1)
    sum3v, area3v = _rect_sum_grid_torch(integral_v, y0, y1, x0, x1)
...
def _detect_mosaic_mask_torch(
    ...
    print("_detect_mosaic_mask_torchがnodeBで呼び出された！")
...
def _detect_mosaic_mask_torch_batch(
    batch_rgb: torch.Tensor, config: MosaicDetectionConfig, max_workers: int
) -> Tuple[List[np.ndarray], List[int]]:
    if batch_rgb.dtype != torch.float32:
        batch_rgb = batch_rgb.to(dtype=torch.float32)

    batch_rgb = torch.clamp(batch_rgb, 0.0, 1.0)

    if batch_rgb.ndim != 4 or batch_rgb.shape[-1] != 3:
        raise ValueError("Expected RGB tensor with shape (N, H, W, 3)")

    batch_size, height, width, _ = batch_rgb.shape

    small_height = max(1, height // 2)
    small_width = max(1, width // 2)

    rgb_chw = batch_rgb.permute(0, 3, 1, 2)
    rgb_small = F.interpolate(
        rgb_chw, size=(small_height, small_width), mode="nearest"
    ).permute(0, 2, 3, 1)
    ...
    mask_small = mask_small * 255

    v_channel = hsv_small[..., 2]
    feature_map = _get_mosaic_feature_map_torch(v_channel, config)
    if feature_map.ndim == 2:
        feature_map = feature_map.unsqueeze(0)
    feature_map = torch.minimum(feature_map, mask_small)

    feature_binary = (feature_map > 0).to(dtype=torch.float32)

    with torch.inference_mode():
        tile_sizes = _estimate_mosaic_period_torch(feature_binary, config)
        mask_small_batch = _assemble_mosaic_mask_torch(
            feature_binary, tile_sizes, config
        )

        if mask_small_batch.ndim == 2:
            mask_small_batch = mask_small_batch.unsqueeze(0)

        mask_full_torch = F.interpolate(
            mask_small_batch.unsqueeze(1),
            size=(height, width),
            mode="nearest",
        ).squeeze(1)

        mask_full_torch = (mask_full_torch > 0.5).to(torch.uint8) * 255

    full_masks: List[np.ndarray] = []
    sizes: List[int] = []

    tile_sizes = tile_sizes.to(dtype=torch.int32).reshape(-1)

    for idx in range(batch_size):
        size_val = int(tile_sizes[min(idx, tile_sizes.numel() - 1)].item())
        if size_val <= 0:
            full_masks.append(np.zeros((height, width), dtype=np.uint8))
            sizes.append(0)
            continue

        mask_np = mask_full_torch[idx].detach().cpu().numpy()
        full_masks.append(mask_np)
        sizes.append(int(size_val * 2))

    return full_masks, sizes
...
class MosaicDetectionNode:
    ...
    def execute(
        ...
        if backend_choice == "AUTO":
            use_torch_backend = torch.cuda.is_available()
        elif backend_choice == "TORCH":
            use_torch_backend = True
        else:
            use_torch_backend = False

        if use_torch_backend and batch_tensor.device.type == "cpu" and torch.cuda.is_available():
            batch_tensor = batch_tensor.to(device="cuda", non_blocking=True)
        ...
        if use_torch_backend:
            mask_list, size_list = _detect_mosaic_mask_torch_batch(
                batch_tensor, config, max_workers
            )
            masks = [mask.astype(np.float32) / 255.0 for mask in mask_list]
            sizes = [int(size) for size in size_list]
        else:
